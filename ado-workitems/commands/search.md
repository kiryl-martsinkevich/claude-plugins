...
3. **Execute the WIQL query:**
   ```bash
   RESULT=$(bash "${CLAUDE_PLUGIN_ROOT}/scripts/ado-api.sh" query "<WIQL>")
   ```
...
6. **Fetch work item details** if IDs were returned:
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/ado-api.sh" batch-get "$IDS"
   ```
...
- Pipe jq output through formatting for readability
