...
Manage Azure DevOps work items through the REST API using a helper script and curl. Supports the Agile process template hierarchy: Epic > Feature > User Story > Task/Bug.
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/ado-api.sh help
```

## Helper Script

All API operations use `${CLAUDE_PLUGIN_ROOT}/scripts/ado-api.sh`. The script handles authentication, URL construction, error handling, and JSON formatting.
...
```bash
SCRIPT="${CLAUDE_PLUGIN_ROOT}/scripts/ado-api.sh"

# Get a work item
bash "$SCRIPT" get 12345 | jq .

# Create a User Story
bash "$SCRIPT" create "User Story" '[
...
# Search with WIQL
bash "$SCRIPT" query "SELECT [System.Id],[System.Title] FROM WorkItems WHERE [System.WorkItemType]='User Story' AND [System.State]='Active' ORDER BY [System.CreatedDate] DESC"
```
...
RESULT=$(bash "$SCRIPT" query "SELECT [System.Id] FROM WorkItems WHERE [System.AssignedTo]=@me AND [System.State]<>'Closed' ORDER BY [System.ChangedDate] DESC")

# Step 2: Extract IDs and fetch details (skip batch-get if no results)
IDS=$(echo "$RESULT" | jq -r '[.workItems[].id] | join(",")')
[[ -n "$IDS" ]] && bash "$SCRIPT" batch-get "$IDS" | jq '.value[] | {id: .id, title: .fields["System.Title"], state: .fields["System.State"], type: .fields["System.WorkItemType"]}'
```
...
```bash
bash "$SCRIPT" add-parent <story-id> <feature-id>
```
...
1. **Fetch the feature**: `bash "$SCRIPT" get <feature-id>` — extract title, description, acceptance criteria
2. **Check for BRD attachments**: `bash "$SCRIPT" attachments <feature-id>` — look for .docx/.pdf files
...
1. **List attachments**: `bash "$SCRIPT" attachments <work-item-id>`
2. **Identify BRD files**: Filter for `.docx` and `.pdf` extensions
3. **Download to temp**: `bash "$SCRIPT" download <attachment-url> /tmp/brd-<id>.<ext>`
...
doc = Document('/tmp/brd-file.docx')
...
- **`${CLAUDE_PLUGIN_ROOT}/scripts/ado-api.sh`** — Bash helper wrapping all ADO REST API operations with auth and error handling
